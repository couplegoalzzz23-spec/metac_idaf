...
# Abaikan peringatan SSL saat mengakses server BMKG
...
def render_generic_page(title, filename, param_key, chart_type='bar', colorscale="Blues", legend_title="Kategori"):
    ...
    if df is not None and not df.empty:
        ...
        st.warning(f"⚠️ Data `{filename}` tidak ditemukan atau belum diunggah ke direktori.")
...
def sanitize_pdf_text(text):
    """Pembersih karakter non-Latin1 agar tidak crash saat rendering FPDF."""
    if not text:
        return "NIL"
    replacements = {'—': '-', '–': '-', '“': '"', '”': '"', '‘': "'", '’': "'", '•': '*', '…': '...', '\r': ''}
    text = str(text)
...
def generate_pdf(data, raw_taf, icao, name=""):
    ...
    def add_fixed_row(label_lines, value_lines, h):
        ...
        pdf.set_font("helvetica", 'B', 10)
        pdf.set_xy(x + 2, y + 2)
        for line in label_lines: pdf.cell(91, 5, sanitize_pdf_text(line), ln=2)
        pdf.set_font("helvetica", '', 10)
        pdf.set_xy(x + 97, y + 2)
        for line in value_lines: pdf.cell(91, 5, sanitize_pdf_text(line), ln=2)
        pdf.set_xy(x, y + h)
    add_fixed_row(["AERODROME IDENTIFICATION"], [icao], 10)
    add_fixed_row(["SURFACE WIND DIRECTION, SPEED", "AND SIGNIFICANT VARIATION"], [data.get('wind', 'NIL')], 12)
    add_fixed_row(["HORIZONTAL VISIBILITY"], [data.get('vis', 'NIL')], 10)
    add_fixed_row(["RUNWAY VISUAL RANGE"], ["NIL"], 10)
    add_fixed_row(["PRESENT WEATHER"], [data.get('wx', 'NIL')], 10)
    add_fixed_row(["AMOUNT AND HEIGHT OF BASE", "OF LOW CLOUD"], [data.get('cld', 'NIL')], 12)
    add_fixed_row(["AIR TEMPERATURE AND", "DEW POINT TEMPERATURE"], [data.get('tt_td', 'NIL')], 12)
    add_fixed_row(["QNH"], [data.get('qnh', 'NIL')], 10)
    add_fixed_row(["QFE*"], [data.get('qfe', 'NIL')], 10)
    pdf.set_font("helvetica", '', 10)
    supp_label = "SUPPLEMENTARY\nINFORMATION"
    supp_val = f"RMK: {data.get('rmk', 'NIL')}\nTREND: {data.get('trend', 'NOSIG')}\n\nTAFOR:\n{raw_taf}"
    supp_val = sanitize_pdf_text(supp_val)
    ...
    # KUNCI PERBAIKAN: Output FPDF Universal (Tahan Banting fpdf / fpdf2)
    try:
...
def generate_raw_pdf(lines):
    content = "BT\n/F1 10 Tf\n72 800 Td\n"
    for l in lines:
        safe = str(l).replace("\\", "\\\\").replace("(", "\\(").replace(")", "\\)")
